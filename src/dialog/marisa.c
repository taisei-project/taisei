...
DIALOG_TASK(marisa, Stage2PreBoss) {
	...
	MSG(marisa, "What could be causin' the youkai to get all weird, anyways?");
...
DIALOG_TASK(marisa, Stage4PreBoss) {
	...
	FACE(marisa, happy);
	MSG(marisa, "Girls across Gensokyo talk about 'em.");
	FACE(marisa, normal);
...
DIALOG_TASK(marisa, Stage4PostBoss) {
	...
	MSG(marisa, "I'm Gensokyo's... petticoat inspector.");
	...
	FACE(kurumi, defeated);
	MSG(kurumi, "Uuu--... the residents of this Gensokyo were supposed to be kept 'occupied'!");
...
DIALOG_TASK(marisa, Stage6PreBoss) {
	...
	MSG(marisa, "Do ya know how boring it is to be a magician in Gensokyo these days?");
	...
	MSG(elly, "But there's no place for sorcery in our enlightened vision of Gensokyo!");
	FACE(elly, shouting);

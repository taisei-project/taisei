...
DIALOG_TASK(marisa, StageExPreBoss) {
	...
	FACE(marisa, normal);
	FACE(yumemi, sad);
	MSG(yumemi, "The public didn’t care. The world continued to burn.");
	MSG(yumemi, "’Big deal. Rich people can already go to the moon. It’s just another toy for them.’ That’s what they’d say.");
	MSG(yumemi, "Occasionally, some politician or CEO would beg me for access…");
	FACE(yumemi, eyes_closed);
	MSG(yumemi, "But I wouldn’t dare give those fools even a glimpse.");

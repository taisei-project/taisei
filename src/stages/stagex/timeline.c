...
static void yumemi_slave_aimed_lasers(YumemiSlave *slave, real s) {
	...
	play_sfx_delayed("laser1", 3, 1, 50);
...
TASK(yumemi_opening, { BoxedBoss boss; Attack *attack; }) {
	Boss *boss = init_boss_attack_task(ARGS.boss, ARGS.attack);
...
DEFINE_EXTERN_TASK(stagex_timeline) {
	...
	boss_start_attack(global.boss, boss->attacks);
